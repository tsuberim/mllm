...
class KVCache:
    ...
    """
    def __init__(self, n_head: int, head_dim: int, max_T: int, dtype=mx.float32, batch_size: int = 1):
        self.offset = 0
        self.k = mx.zeros((batch_size, n_head, max_T, head_dim), dtype=dtype)
        self.v = mx.zeros((batch_size, n_head, max_T, head_dim), dtype=dtype)
...
class GPT(nn.Module):
    ...
    def make_cache(self, max_T: int | None = None, batch_size: int = 1) -> list[KVCache]:
        dtype = self.tok_emb.weight.dtype
        T = max_T if max_T is not None else self.cfg.block_size
        return [KVCache(b.attn.n_kv_head, b.attn.head_dim, T, dtype=dtype, batch_size=batch_size)
